...
export default function PendingMembersPage() {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <p style={{ color: '#9C8A76', marginBottom: '24px' }}>{members.length} waiting</p>
            ...
            <div key={m.id} style={{ background: 'white', padding: '20px', borderRadius: '12px', border: '1px solid #E0E0E0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  style={{ padding: '10px 16px', background: '#2E7D32', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 700, fontSize: '13px' }}
                >
                  Approve
                </button>
                <button
                  onClick={() => reject(m.id)}
                  disabled={processing === m.id}
                  style={{ padding: '10px 16px', background: '#D32F2F', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 700, fontSize: '13px' }}
                >
                  Reject
                </button>
              </div>
