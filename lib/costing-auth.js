...
export async function requireRole(request, ...allowedRoles) {
  ...
  if (!session) {
    ...
    if (mainToken) {
      if (mainToken === 'admin_pin_session' || String(mainToken).startsWith('admin_pin_')) {
        session = { role: 'admin', user_id: 'admin_pin_user' }
      } else {
  ...
  // Also check direct admin PIN header or cookie
  if (!session) {
  ...
  if (!session) {
    const cookieHeader = request.headers.get('cookie') || ''
    if (cookieHeader.includes('cc_costing_token=admin_costing_session') || cookieHeader.includes('isAdmin=true')) {
      session = { role: 'admin', user_id: 'admin_pin_user' }
    }
  }
