...
async function callAISafe(prompt) {
  try {
    ...
    console.warn('[attendance-agent] Gemini failed, trying Anthropic...', err.message)
    return await callAIFallback(prompt)
  }
}

// ── 1. Draft Weekly Roster ────────────────────────────────────────────────────

/**
 * Generate an AI-proposed roster for the given week.
 * Returns a draft that is saved to ai_roster_drafts with status='pending'.
 * Admin must approve before any duty_roster rows are written.
 *
 * @param {string} weekStart - YYYY-MM-DD (Monday)
 */
export async function draftWeeklyRoster(weekStart) {
  // Fetch all active staff with their preferences (anonymised for AI)
  ...
  if (!staffList?.length) throw new Error('No active staff found')
  ...
  const staffContext = staffList.map((s, idx) => {
    ...
    return {
      ...
      shift_preference: s.shift_start || '10:00',
  ...
  const prompt = `You are a workforce scheduling assistant for a coffee shop. Generate a 7-day duty roster.

RULES:
1. Each employee works exactly 6 days per week (1 day off).
2. The day off should match their "weekly_off_day" preference unless they have approved leave.
3. Default shift is 10:00 and lasts 10 hours.
4. If an employee has approved_leave_dates, mark those days as "leave" not "off".
5. Distribute workload fairly — employees with more recent hours should be given lighter shifts if possible.
6. Employees with a high late_count_last_4_weeks should NOT be scheduled for early shifts.
7. Ensure at least 60% of staff are scheduled to work each day (no understaffed days).

WEEK: ${weekStart} to ${days[6]}
