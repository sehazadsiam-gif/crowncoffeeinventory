...
async function callAI(prompt, jsonMode = true) {
  ...
  for (const modelName of candidateModels) {
    try {
      ...
      console.warn(`[attendance-agent] Model ${modelName} failed:`, err.message)
      lastErr = err
    }
  }
...
export async function draftWeeklyRoster(weekStart) {
  ...
  const staffContext = staffList.map((s, idx) => {
    ...
    return {
      ...
      shift_preference: s.shift_start || '11:00',
...
4. Official shift start times are 11:00 (11:00 AM) and 13:00 (1:00 PM).
   - Cutoff boundary for unrostered check-in classification: <12:30 PM -> 11:00, >=12:30 PM -> 13:00.
