...
/**
 * Calculate Online Channel Metrics:
 * - Online Listed Price
 * - Customer Discount % (Disc%)
 * - Customer Price = Listed Price * (1 - Disc%)
 * - App Commission % (Comm%)
 * - Net Payout from App = Customer Price * (1 - Comm%)
 * - Base Cost = Making Cost + Utilities (1:1)
 * - Online Net Profit (৳) = Net Payout - Base Cost
 */
export function calculateOnlineChannelMetrics(listedPrice, cogs, commissionPct, discountPct = 0, utilitiesRatio = 1.0) {
  ...
  const customerPrice = price * (1 - (disc / 100))
  const commissionAmt = customerPrice * (comm / 100)
  const netPayout = customerPrice - commissionAmt

  const onlineProfit = price > 0 ? netPayout - baseCost : 0
  const onlineProfitPct = price > 0 ? (onlineProfit / price) * 100 : 0
  const netFoodCostPct = netPayout > 0 ? (makingCost / netPayout) * 100 : 0

  const isLoss = price > 0 && netPayout < baseCost
  const isThinProfit = price > 0 && !isLoss && onlineProfit < makingCost * 0.5
