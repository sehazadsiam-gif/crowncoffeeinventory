...
export async function logAttendance({
  ...
  try {
    ...
    // ── 2-Minute (120s) Double-Tap Cooldown Guard ──
    const tapKey = `${staff.id}_${dateStr}`
    const lastTapTime = lastTapMap.get(tapKey)
    if (lastTapTime && (checkInAt.getTime() - lastTapTime) < 120000 && !adminOverride) {
      const remainingSecs = Math.ceil((120000 - (checkInAt.getTime() - lastTapTime)) / 1000)
      return {
        success: false,
        blocked: true,
    ...
    // 1. Resolve Shift Start & Rules (8:00 AM System vs 1:00 PM System)
    // Morning Shift (8:00 AM - 12:00 PM): Shift Start = 8:00 AM (480m), Grace = 8:15 AM (495m)
    // Afternoon Shift (After 12:00 PM): Shift Start = 1:00 PM / 13:00 (780m), Grace = 1:15 PM / 13:15 (795m)
    let shiftStartStr = '08:00'
    let shiftStartMinutes = 8 * 60       // 8:00 AM = 480 mins
    let deadlineMinutes   = 8 * 60 + 15  // 8:15 AM = 495 mins

    if (checkInMinutes >= 12 * 60) {
      shiftStartStr = '13:00'
      shiftStartMinutes = 13 * 60      // 1:00 PM = 780 mins
      deadlineMinutes   = 13 * 60 + 15 // 1:15 PM = 795 mins
    }
    ...
    if (existing && existing.check_in_at && !existing.check_out_at && !adminOverride) {
      ...
      // ── Safety Cooldown Guard: Prevent double-taps within 120s of check-in ──
      ...
      if (elapsedSinceCheckIn >= 0 && elapsedSinceCheckIn < 120000) {
        const remainingSecs = Math.ceil((120000 - elapsedSinceCheckIn) / 1000)
        return {
          success: false,
          blocked: true,
...
export async function autoFlagAbsent(dateStr) {
  ...
  try {
    ...
    for (const s of allStaff) {
      ...
      // ── No day-off / roster logic: every active staff with no check-in is absent ──
      inserts.push({
        staff_id: s.id,
