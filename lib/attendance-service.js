...
export async function logAttendance({
  ...
  try {
    ...
    // ── 10-Second Debounce Guard for Readers ──
    const tapKey = `${staff.id}_${dateStr}`
    const lastTapTime = lastTapMap.get(tapKey)
    if (lastTapTime && (checkInAt.getTime() - lastTapTime) < 10000 && !adminOverride) {
      return {
        success: false,
        blocked: true,
        error: 'Please wait 10 seconds between card scans.',
    ...
    const adminOverride = !!manualStatus
    ...
    if (existing && existing.check_in_at && !existing.check_out_at && !adminOverride) {
      // Tap 2 (Break Start): if enableBreak is requested and no break started yet
      ...
      // Tap 3 (Break End): if break was started but not ended yet
      ...
      // Tap 4 or Tap 2 (Standard Check-Out)
