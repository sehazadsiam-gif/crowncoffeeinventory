...
export async function logAttendance({
  ...
  try {
    ...
    const localYear = checkInAt.getFullYear()
    const localMonth = String(checkInAt.getMonth() + 1).padStart(2, '0')
    const localDay = String(checkInAt.getDate()).padStart(2, '0')
...
export async function logCheckOut({ identifier, timestamp, source = 'manual' }) {
  try {
    ...
    const localYear = checkOutAt.getFullYear()
    const localMonth = String(checkOutAt.getMonth() + 1).padStart(2, '0')
    const localDay = String(checkOutAt.getDate()).padStart(2, '0')
    const dateStr = `${localYear}-${localMonth}-${localDay}`
