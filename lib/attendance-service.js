...
async function getRosterForDate(staffId, dateStr) {
  ...
  return {
    shift_start: staff.shift_start || '11:00',
    is_off: isOff,
    is_leave: false,
    shift_hours: 10,
...
export async function logAttendance({
  ...
  try {
    ...
    // Shifts are strictly 11:00 AM (11:00) and 1:00 PM (13:00):
    // - Taps before 12:30 PM (<750m) -> 11:00 AM Shift (11:00), Grace until 11:15 AM (675m)
    // - Taps at/after 12:30 PM (>=750m) -> 1:00 PM Shift (13:00), Grace until 1:15 PM (795m)
    let shiftStartStr = '11:00'
    let shiftStartMinutes = 11 * 60
...
export async function autoFlagAbsent(dateStr) {
  ...
  try {
    ...
    for (const s of allStaff) {
      ...
      const effectiveShiftStart = roster?.shift_start || s.shift_start || '11:00'
...
/**
 * Inject synthetic 10-hour work logs for August 1 to August 7, 2026 for any active staff lacking a real DB log.
 */
export function injectAugustBaselineLogs(existingLogs = [], staffList = [], startDateStr = '', endDateStr = '') {
  ...
  existingLogs.forEach(l => {
    if (l.staff_id && l.date) {
      logsMap.add(`${l.staff_id}_${l.date}`)
    }
  })

  const syntheticLogs = []

  BASELINE_DATES.forEach(dateStr => {
    ...
    staffList.forEach(s => {
      ...
      if (!logsMap.has(key)) {
        syntheticLogs.push({
          id: `auto-aug1-8-${s.id}-${dateStr}`,
          staff_id: s.id,
          employee_id: s.employee_id || 'N/A',
          date: dateStr,
          status: 'present',
          check_in_at: `${dateStr}T11:00:00+06:00`,
          check_out_at: `${dateStr}T21:00:00+06:00`,
          hours_worked: 10.0,
          overtime_hours: 0,
          overtime_minutes: 0,
          minutes_late: 0,
          ...
          notes: 'Auto-credited (Aug 1 - Aug 8 baseline)'
        })
      }
    })
  })

  return [...existingLogs, ...syntheticLogs].sort((a, b) => new Date(a.date) - new Date(b.date))
}
