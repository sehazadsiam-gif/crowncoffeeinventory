...
export async function logAttendance({
  ...
  try {
    ...
    // ── Bangladesh Local Time Calculation (Asia/Dhaka UTC+6) ──
    ...
    const bdParts = Object.fromEntries(bdFormatter.formatToParts(checkInAt).map(p => [p.type, p.value]))
    const dateStr = `${bdParts.year}-${bdParts.month}-${bdParts.day}`

    const localHour = parseInt(bdParts.hour, 10) % 24
    const localMin = parseInt(bdParts.minute, 10)
    ...
    if (adminOverride && forceStatus) {
      ...
      } else if (checkInMinutes < 5 * 60) {
        // Very early morning (e.g. 2:00 AM - 4:59 AM) is considered late for afternoon/night duty
        status = 'late'
        minutesLate = 60
      } else {
        status = 'present'
...
export async function logCheckOut({ identifier, timestamp, source = 'manual' }) {
  try {
    ...
    const bdParts = Object.fromEntries(bdFormatter.formatToParts(checkOutAt).map(p => [p.type, p.value]))
    const dateStr = `${bdParts.year}-${bdParts.month}-${bdParts.day}`
    ...
    const isEarlyDeparture = totalMinutesWorked < 15 || totalMinutesWorked < (standardShiftMinutes - 15)
