...
export async function sendWhatsAppVisitConfirmation(phone, name, totalVisits, punchCount) {
  const remaining = 6 - (punchCount % 6)
  const text = `Crown Coffee: Thank you for visiting, ${name}! Total visits: ${totalVisits}. ${remaining} more punches for a FREE coffee! ☕`
  return sendWhatsAppNotification({ phone, text })
}

export async function sendWhatsAppTierUpgrade(phone, name, tier) {
  const text = `Crown Coffee: 🎉 Congratulations ${name}! You have been upgraded to ${tier} Tier. Enjoy your exclusive benefits on your next visit!`
  return sendWhatsAppNotification({ phone, text })
}