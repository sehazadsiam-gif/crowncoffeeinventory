...
export async function sendVisitRecordedSMS(phone, name, visits, punchCount) {
  const currentPunch = punchCount % 5 === 0 ? 5 : punchCount % 5
  const remaining = 5 - currentPunch
  const message = `Crown Coffee: Thank you for visiting! Total visits: ${visits}. Punch card: ${currentPunch}/5. ${remaining} more punches for a free coffee.`
  return sendSMS(phone, message)
}

export async function sendFreeCoffeeSMS(phone, name, cardNumber) {
  const message = `Crown Coffee: Congratulations! You have earned a free coffee! Card: ${cardNumber}. Valid for 7 days. Redeem your reward today!`
  return sendSMS(phone, message)
}
...
export async function sendAnniversarySMS(phone, name, years) {
  const message = `Crown Coffee: Happy ${years} Year Anniversary, ${name}! Thank you for being a valued member. We have a special gift waiting for you. Visit us today!`
  return sendSMS(phone, message)
}
