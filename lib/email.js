    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 24px;">
          ...
          <a href="https://ccadmin.online" style="color: #6B3A2A;">ccadmin.online</a>
          <br/><br/>
...
export async function sendFeedbackRequest({ to, name, visit_id }) {
  if (!to) return
  try {
    const baseUrl = 'https://ccadmin.online/api/members/feedback'
    await transporter.sendMail({
      from: `Crown Coffee <${process.env.GMAIL_USER}>`,
      to,
...
export async function sendMembershipCardEmail(member, cardNumber) {
  const membershipCardHTML = `
    <div style="font-family: 'Segoe UI', sans-serif; background: #FAFAFA; padding: 40px 20px;">
      <div style="max-width: 600px; margin: 0 auto;">
        
        <!-- Header -->
        <div style="text-align: center; margin-bottom: 32px;">
          <h1 style="color: #6B3A2A; font-size: 28px; margin: 0 0 8px 0; letter-spacing: -0.5px;">
            Welcome to Crown Coffee Family
          </h1>
          <p style="color: #9C8A76; margin: 0; font-size: 14px;">Your membership is active now</p>
        </div>
        ...
        <div style="background: white; border: 1px solid #E0E0E0; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
          ...
          <div style="display: flex; gap: 16px; margin-bottom: 12px;">
            <div style="color: #2E7D32; font-size: 20px;">✓</div>
            <div>
              <div style="color: #1F1F1F; font-weight: 700; margin-bottom: 4px;">Free Coffee Every 10 Visits</div>
          ...
          <div style="display: flex; gap: 16px;">
            <div style="color: #2E7D32; font-size: 20px;">✓</div>
            <div>
              <div style="color: #1F1F1F; font-weight: 700; margin-bottom: 4px;">Upgrade to Gold at 25 Visits</div>
...
export async function sendVisitConfirmationEmail(member, freeCoffeeProgress, goldUpgradeProgress) {
  const freeCoffeeLeft = 10 - freeCoffeeProgress.current_punch
  const visitsLeft = 25 - member.total_visits
  const goldUpgradeEligible = member.total_visits >= 25

  const visitConfirmationHTML = \`
    <div style="font-family: 'Segoe UI', sans-serif; background: #FAFAFA; padding: 40px 20px;">
      <div style="max-width: 600px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin-bottom: 32px;">
          ...
          <p style="color: #9C8A76; margin: 0; font-size: 14px;">Thank you for visiting Crown Coffee</p>
        </div>
        ...
        \${!goldUpgradeEligible ? \`
          <div style="background: #FFF3E0; border: 1px solid #FFE0B2; border-radius: 12px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #F57C00;">
            ...
            <div style="background: #E0E0E0; height: 8px; border-radius: 4px; overflow: hidden; margin-bottom: 12px;">
              <div style="background: #F57C00; height: 100%; width: \${Math.round((member.total_visits / 25) * 100)}%; transition: width 0.3s;"></div>
            </div>
            ...
            <div style="text-align: center;">
              ...
              <div style="color: #9C8A76; font-size: 13px;">
                Reach 25 visits to upgrade to Gold tier and enjoy 10% lifetime discount!
              </div>
            </div>
          </div>
        \` : \`
