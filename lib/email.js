...
export async function sendRemarkEmail({ to, name, note_type, note, date }) {
  if (!to) return
  try {
    console.log('Sending Remark email to:', to)
    ...
    console.log('Remark email sent successfully to:', to)
  } catch (err) {
    console.error('Failed to send Remark email:', err)
  }
}
...
export async function sendPayrollEmail({ to, name, month, year, breakdown }) {
  if (!to) return
  try {
    console.log('Sending Payroll email to:', to)
    await resend.emails.send({
      from: 'Crown Coffee <onboarding@resend.dev>',
      to,
    ...
    console.log('Payroll email sent successfully to:', to)
  } catch (err) {
    console.error('Failed to send Payroll email:', err)
  }
}
...
export async function sendAdvanceEmail({ to, name, amount, reason, date }) {
  if (!to) return
  try {
    console.log('Sending Advance email to:', to)
    await resend.emails.send({
      from: 'Crown Coffee <onboarding@resend.dev>',
      to,
    ...
    console.log('Advance email sent successfully to:', to)
  } catch (err) {
    console.error('Failed to send Advance email:', err)
  }
}
