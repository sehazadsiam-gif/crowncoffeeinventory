...
async function run() {
  ...
  for (const member of members) {
    ...
    if (member.tier === 'silver' && member.total_visits >= 11) {
      console.log(`Upgrading ${member.full_name} to Gold...`);
      ...
      if (!updateError) {
        ...
        console.error(`Upgrade failed for ${member.full_name}:`, updateError);
      }
    }
    ...
    try {
      ...
      console.log(`Sent to ${member.email}${upgraded ? ' (Upgraded)' : ''}`);
    } catch (err) {
